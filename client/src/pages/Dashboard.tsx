    ...
    useEffect(() => {
      const fetchDashboardData = async () => {
        try {
            ...
            setActivities(acctivityRes.data)
        } catch (error: any) {
  ...
  return (
    <div className="space-y-8">
        ...
        <div>
            ...
            <p className="text-slate-900 text-sm mt-0.5">Here's What happening with your social accounts today.</p>
        </div>
            ...
            {statCards.map((card)=>(
                <div key={card.label} className="bg-white hover:bg-red-50 relative border border-slate-200 rounded-2xl p-5 hover:border-red-200 transition-all">
                    <div className="flex items-center justify-between mb-4">
                        <div className="text-3xl font-medium text-slate-800">{card.value}</div>
                        <div className="text-xs absolute right-4 top-4 text-red-500 flex items-center">
    ...
    {activities.map((activity) => (
        <div
            key={activity._id}
            className="grid grid-cols-[auto_1fr_auto] items-center gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors"
        >
            {/* Icon */}
            <div className="size-9 rounded-xl flex items-center justify-center shrink-0 bg-zinc-100 text-zinc-600">
