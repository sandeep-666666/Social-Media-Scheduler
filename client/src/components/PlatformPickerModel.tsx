      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100">
        ...
        <div className="p-6 flex flex-col gap-2">
          {PLATFORMS.map((p)=>{
            ...
            className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${isConnected ? "border-red-200 bg-red-50 cursor-default": "border-slate-50 hover:border-slate-300 hover:bg-slate-100 cursor-pointer"} ${isConnecting && "opacity-60"}`}
               >
              {/* icon */}
              <div className="p-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-500 truncate">
                  {isConnected ? "Alredy Connected": p.description}
                </div>
              </div>
