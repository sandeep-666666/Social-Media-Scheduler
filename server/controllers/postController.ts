...
const pollLeonardoJob = async(generationId: string, apiKey: string): Promise<string> =>{
    ...
    for(let i=0; i<maxRetries; i++){
        try {
            const response = await axios.get(`https://cloud.leonardo.ai/api/rest/v1/generaions/${generationId}`, {headers: {
                accept:"application/json",
                authorization:`Bearer ${apiKey}`
            }})
            const generation = response.data.generaions_by_pk;
...
export const generatePost = async(req: AuthRequest, res: Response):Promise<void> =>{
    try {
        ...
        if(!apiKey){
            res.status(400).json({
                message:"GEMINI_API_KEY is missing to add it to your server/.env file"
            })
            return
        }
        ...
        if(generateImage){
            try {
                const leonardoKey = process.env.LEONADO_API_KEY;
                if(leonardoKey){
...
export const schedulePosts = async(req: AuthRequest, res: Response):Promise<void> =>{
    try {
        ...
        //parse platforms if it comes as stringified array from formData
        ...
        if(req.file){
            const result = await new Promise<any>((resove, reject)=>{
