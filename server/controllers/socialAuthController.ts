...
// Heler to ensure the user has a zernio profile
const getOrCreateZernioProfile = async (user:any): Promise<string> => {
    try {
        ...
        if(!pid){
            throw new Error ("Failed to create Zernio profile - no ID rereturned")
        }
        ...
        for(const zAccount of zernioAccounts){
            ...
            const account = await Account.findOneAndUpdate(
                {zernioAccountId:zid},
                {
                    ...
                    handle:zAccount.usrname || zAccount.name || zAccount.handle || "Unknown",
                    zernioAccountId:zid,
                    status:"connected",
